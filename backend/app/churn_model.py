...
def predict_churn(customer_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        # Model artifacts can be incompatible with the installed scikit-learn
        # version. Surface this as an unavailable model instead of a 500 error.
        raise ChurnModelUnavailable(
            f"Churn model inference is unavailable: {exc}"
        ) from exc
    pred = int(proba >= 0.5)
